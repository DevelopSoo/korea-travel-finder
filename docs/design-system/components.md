> 목차는 `index.md`. 실물은 `/en/dev/ui` 에서 한 화면에 볼 수 있다 (개발 환경에서만 열린다).
...
| 4-17 | `Notice.tsx`                          | 화면 아래 알림줄 (저장 실패·실행 취소)    |
| —    | `icons.tsx`                           | 선 아이콘 모음                            |
...
- 화면 아래 가운데에 뜨는 한 줄 알림. 먹색(`ink`) 바탕 + `paper` 글자 14px, 모서리 6px, 최대 폭 440px. 그림자·움직임은 없다 — 크림 바탕 위 먹색 덩어리로 충분히 구분된다.
- 버튼은 **하나까지**: 글자 링크 모양(밑줄, 600). 저장 실패 → `Try again`, 저장 해제 → `Undo`.
- 6초 뒤 저절로 닫힌다. 마우스를 올리거나 버튼에 초점이 있으면 기다린다. 한 번에 하나만 보인다 — 새 알림이 앞 알림을 바꾼다.
- 레이아웃(`[lang]/layout.tsx`)에 **하나만** 둔다. 띄울 때는 `src/lib/notice.ts` 의 `showNotice(kind, action)` 을 부른다. 문구는 `en.json` → `notice` 에서 kind 로 고른다.
- 성공을 알리는 데 쓰지 않는다. 저장 성공은 책갈피가 채워지는 것으로 충분하다 (§4-6).
- 쓰는 자리: 로그인한 사람의 저장·해제 실패, 계정 목록 불러오기 실패, 저장 페이지의 Remove(실행 취소).
