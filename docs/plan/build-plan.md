...
> ⚠️ **이 계획은 디자인 시스템 v1 기준이며 2026-09-22 에 끝났다.**
> v2 로 전면 교체하면서 1~7단계의 산출물(토큰·폰트·부품 14종·질문/결과 화면)은 모두 다시 만들었다.
> 지금의 기준 문서는 `docs/design-system/*.md` (v2) 다. 아래 내용은 기록으로만 남긴다.
>
...
방법: 390px 스크린샷(`/en/dev/ui` 부품 16개 + `/en` `/en/find` `/en/results` `/en/experiences/[slug]` `/en/privacy` 404)과 `src/` grep 을 함께 대조.
부품 모음은 `src/app/[lang]/dev/ui/page.tsx` + `Gallery.tsx`. 프로덕션(`next build`/`start`)에서는 `NODE_ENV === "production"` 이면 `notFound()` → 404 확인.
스크린샷은 Playwright 캐시의 `chrome-headless-shell` 로 찍었다. 일반 Chrome headless 는 창 최소 폭이 500px 라 390px 로 찍으면 오른쪽이 잘린다.

| 번호 | 항목 | 결과 | 근거 |
| ---- | ---- | ---- | ---- |
| 1 | 그라데이션 | 통과 | `gradient` `bg-linear/radial/conic` 0건. 단색만 |
| 2 | 보라·파랑, 네온 | 통과 | 색은 `@theme` 토큰뿐(`--color-*: initial`). 강조는 쪽빛 하나 |
| 3 | 크림 + 세리프 + 주황 | 통과 | 배경 `#FFFFFF`, 제목 Bricolage, 강조 쪽빛 |
| 4 | 유리 카드 | 통과 | `blur` `backdrop` `opacity` 0건. 흰 카드 + 1px `line` |
| 5 | 그림자 | 통과 | `shadow` 클래스 0건. 첫 화면 글자 그림자만 인라인 예외(photos.md §3-4) — 지금은 사진이 없어 꺼져 있다 |
| 6 | 큰 모서리 | 통과 | `rounded-md`(4px) 7곳뿐 |
| 7 | 아이콘 남발 | 통과 | 아이콘 라이브러리·`<svg>` 0건. ← ↗ ★ 글자만 |
| 8 | 이모지 제목 | 통과 | 이모지 0건 |
| 9 | 큰 숫자 3개 | 통과 | 없음 |
| 10 | 번호 단계 장식 | 통과 | `1 / 3` 은 Progress 에만 |
| 11 | 등장 애니메이션 | 통과 | `animate-` `transition` 0건 |
| 12 | 마우스 올리면 커짐 | 통과 | `hover:` 는 ExperienceCard `hover:border-ink` 하나(테두리 색만). `scale` `translate` 0건 |
| 13 | 뻔한 문구 | 통과 | discover / unlock / curated / magic / journey 0건 |
| 14 | 사진 위 반투명 막 | 통과 | 막 없음. overlay 캡션은 글자만 |
| 15 | AI 생성·스톡 사진 | 통과(해당 없음) | 실제 사진이 아직 없다 — 회색 네모 + 캡션 |
| 16 | 균일한 여백 | 통과 | 사진은 몸통 폭·첫 화면 전체, 카드 안 글은 `xs`~`md` 로 촘촘, 구역 사이만 `xl` |
| 17 | 세리프 제목 | 통과 | 제목 Bricolage Grotesque |
| 18 | Inter / Poppins / Roboto | 통과 | 0건. Bricolage + Plex Sans/Mono |
| 19 | 알약 뱃지 | 통과 | RegionBadge 글자만 대문자, 배경·테두리 없음 |
| 20 | "Powered by AI", ✨ | 통과 | `AI` `Powered` ✨ 0건 |
| 21 | 가짜 리뷰 수 | 통과 | review / rating 0건. ★ 는 `EN` `Local` 사실 정보만 |
| 22 | 발에 4열 링크 | 통과 | 링크 3개 한 줄 |
| 23 | 원형 아이콘 로고 | 통과 | 글자 로고만(임시 이름) |
| 24 | 모든 카드 같은 높이 | 통과 | 높이 고정·`line-clamp`·`truncate` 0건. 결과 화면 카드 높이가 글 길이대로 다르다 |

**위반 0.** 덧붙여 확인: `error` 색은 `EmailBox` 안에서만 쓰인다(3곳). 굵기는 400·500·600 만.

**자체 점검 3문항**

1. 사진을 전부 빼면 뭐가 남나? → 지금 화면이 곧 "사진을 뺀" 상태다. 남는 것은 글, 1px 선, 화면마다 검은 큰 버튼 하나뿐 — 장식이 없어 정상. 대신 첫 화면과 카드가 회색 네모로 비어 보인다. 사진이 주인공이라는 뜻이고, 출시 전 실제 사진 채우기가 필수다.
2. 다른 여행 사이트 옆에 놓으면 우리 것인지 알겠나? → Mono 캡션(`Jumunjin, Gangneung · 05:40`)과 카드의 `Why here, not Seoul` 소제목이 표시다. 다만 사진이 없는 지금은 캡션 혼자서는 약하다 — 실제 사진이 들어간 뒤 다시 본다.
3. 화면에서 하나를 뺄 수 있나? → 문서(structure.md·components.md)에 없는 요소는 화면에 없다. 뺄 것 없음.

**미결 2번 (쪽빛 vs 강릉 바다 사진)**: 보류. 실제 사진이 없어(`public/photos/` 없음, 샘플 `image_urls` 전부 빈 배열) 확인할 수 없다. 사진 추가 후 다시 확인한다.
참고로 지금 쪽빛이 쓰이는 곳(★, 글자 링크, 선택 버튼, 이메일 칸 focus)은 모두 흰 바탕 위이고, 사진 위에 올라가는 글자(캡션·첫 화면 문구)는 흰색이라 사진 위에 쪽빛이 놓이는 자리는 현재 없다.
